...
class Skim_pengabdian_model extends CI_Model
{
	var $table = 'mst_skim';
	...
	private function _get_datatables_query()
	{
		...
		$this->db->where('a.id_tipe_pengajuan', '2');
		$i = 0;
	...
	function insert_skim_pengabdian($table, $data)
	{
		$insert = $this->db->insert($table, $data);
		return $insert;
	}

	function update_skim_pengabdian($id, $data)
	{
		$this->db->where('id', $id);
		$this->db->update('mst_skim', $data);
	}

	function get_skim_pengabdian($id)
	{
		$this->db->where('id', $id);
		return $this->db->get('mst_skim')->row();
	}

	function delete_skim_pengabdian($id, $table)
	{
		$this->db->where('id', $id);
