...
class Luaran_lain_penelitian_model extends CI_Model
{
	...
	var $column_order = array('nama', 'nidn', 'judul', 'jenis', 'deskripsi', 'tanggal', 'file', 'status');
	...
	// 	$this->db->like('luaranlain.judul', $term);
	// 	$this->db->or_like('luaranlain.jenis', $term);
	// 	$this->db->or_like('dosen.nama', $term);
	// 	$this->db->or_like('refpengajuan.nama_tipe_pengajuan', $term);
	// 	$this->db->where('luaranlain.id_tipe_pengajuan', '1');
	private function _get_datatables_query($term = '')
	{
		$this->db->select('luaranlain.*, dosen.gelar_depan, dosen.nama, dosen.gelar_belakang, dosen.nidn, refpengajuan.nama_tipe_pengajuan');
		$this->db->from('tbl_luaran_lain as luaranlain'); //luaranlain
