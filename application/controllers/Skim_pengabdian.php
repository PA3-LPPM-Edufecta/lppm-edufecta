...
class Skim_pengabdian extends MY_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model(array('skim_pengabdian_model', 'luaran_model', 'akses_model'));
    }
    ...
    public function index()
    {
        ...
        if ($view_level == 'Y') {
            return $this->template->load('admin/layouts/layoutbackend', 'admin/masterdata/skim_pengabdian', $data);
        } else {
    ...
    public function ajax_list()
    {
        ...
        //set_time_limit(3600);
        $list = $this->skim_pengabdian_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $n = 1;
        foreach ($list as $skimpengabdian) {
            $no++;
            $row = array();
            $row[] = $n++;  
            $row[] = $skimpengabdian->nama;
            $row[] = $skimpengabdian->keterangan;
            $row[] = $skimpengabdian->maksimal_pengajuan;
            $row[] = $skimpengabdian->syarat;
            $row[] = $skimpengabdian->lama_penyelesaian;
            $row[] = $skimpengabdian->nama_tipe_pengajuan;
            $row[] = $skimpengabdian->status;
            $row[] = $skimpengabdian->id;
            $data[] = $row;
        }

        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->skim_pengabdian_model->count_all(),
            "recordsFiltered" => $this->skim_pengabdian_model->count_filtered(),
            "data" => $data,
        );
    ...
    public function insert()
    {
        ...
        $this->skim_pengabdian_model->insert_skim_pengabdian("mst_skim", $save);
    ...
    public function update()
    {
        ...
        $this->skim_pengabdian_model->update_skim_pengabdian($id, $data);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function edit_skim_pengabdian($id)
    {
        $data = $this->skim_pengabdian_model->get_skim_pengabdian($id);
    ...
    {
        $id = $this->input->post('id');
        $this->skim_pengabdian_model->delete_skim_pengabdian($id, 'mst_skim');
        echo json_encode(array("status" => TRUE));
    }
