...
class Buku_ajar extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model(array('Buku_ajar_model', 'Data_dosen_model', 'Tipe_pengajuan_model', 'akses_model'));
    }
    ...
    public function index()
    {
        ...
        $data['id_buku'] =  $this->Buku_ajar_model->getLastId()->result();
        ...
        if ($view_level == 'Y') {
            return $this->template->load('admin/layouts/layoutbackend', 'admin/kinerja_penelitian/buku_ajar', $data);
        } else {
    ...
    public function ajax_list()
    {
        ...
        //set_time_limit(3600);
        $list = $this->Buku_ajar_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Buku_ajar_model->count_all(),
            "recordsFiltered" => $this->Buku_ajar_model->count_filtered(),
            "data" => $data,
        );
            ...
            if ($this->upload->do_upload('file_editorial_board')) {
                ...
                if ($this->upload->do_upload('file_penerbit')) {
                    ...
                    if ($this->upload->do_upload('file')) {
                        ...
                        $this->Buku_ajar_model->insert_buku("tbl_buku", $save);
                        $this->Buku_ajar_model->insert_bukulain("tbl_buku_file_lain", $save2);
    ...
    public function update()
    {
        ...
        if ($this->upload->do_upload('file_cover')) {
            ...
            $this->Buku_ajar_model->update_buku($id, $save);
            ...
            if ($this->upload->do_upload('file_editorial_board')) {
                ...
                if ($this->upload->do_upload('file_penerbit')) {
                    ...
                    if ($this->upload->do_upload('file')) {
                        ...
                        $this->Buku_ajar_model->update_buku($id, $save);
                        $this->Buku_ajar_model->update_bukulain($id, $save2);
                        echo json_encode(array("status" => TRUE));
                    } else {
                        $save = array(
                            'id_tipe_pengajuan'     => 1,
    ...
    public function update_status()
    {
        ...
        $this->Buku_ajar_model->update_buku($id, $data);
        echo json_encode(array("status" => TRUE));
    }

    public function edit_buku($id)
    {
        $data = $this->Buku_ajar_model->get_buku($id);
    ...
    // {
    // 	$buku = $this->Buku_ajar_model->get_buku($id);
    // 	$status = $this->input->post('status');
    ...
    // 		'status'	=> $status
    // 	);
    // 	$this->Buku_ajar_model->edit_status($id, $save);
    //     echo json_encode(array("status" => TRUE));
    // }
    ...
    public function delete()
    {
        ...
        // $g = $this->Buku_ajar_model->getImage1($id)->row_array();
        // $h = $this->Buku_ajar_model->getImage2($id)->row_array();
        // $i = $this->Buku_ajar_model->getImage3($id)->row_array();
        // $j = $this->Buku_ajar_model->getImage4($id)->row_array();
        ...
        $this->Buku_ajar_model->delete_buku($id, 'tbl_buku');
        $this->Buku_ajar_model->delete_buku_file_lain($id, 'tbl_buku_file_lain');
        $data['status'] = TRUE;
        echo json_encode($data);
    }
