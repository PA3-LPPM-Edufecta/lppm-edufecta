...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $("#tbl_buku").DataTable({
			...
			"ajax": {
				"url": "<?php echo site_url('buku_ajar/ajax_list') ?>",
				"type": "POST"
			},
			...
			"columnDefs": [{
				...
				// 			return `
				// 				<div class="text-center">
				// 					<a class="btn btn-xs btn-outline-primary btn-block" href="<?php echo base_url("assets/uploads/foto/cover/"); ?>` + row[13] + `" data-toggle="lightbox" data-title="View File"><i class="fa fa-eye" aria-hidden="true"></i> View</a>
				// 					<a class="btn btn-xs btn-outline-dark btn-block" href="<?php echo base_url("assets/uploads/foto/cover/"); ?>` + row[13] + `" download><i class="fa fa-download" aria-hidden="true"></i> Download</a>
				// 				</div>
				// 			`;
				// 		} else {
				// 			return `
				...
				// 			return `
				// 				<div class="text-center">
				// 					<a class="btn btn-xs btn-outline-primary btn-block" href="<?php echo base_url("assets/uploads/foto/cover/"); ?>` + row[14] + `" data-toggle="lightbox" data-title="View File"><i class="fa fa-eye" aria-hidden="true"></i> View</a>
				// 					<a class="btn btn-xs btn-outline-dark btn-block" href="<?php echo base_url("assets/uploads/foto/cover/"); ?>` + row[14] + `" download><i class="fa fa-download" aria-hidden="true"></i> Download</a>
				// 				</div>
				// 			`;
				// 		} else {
				// 			return `
				...
				// 			return `
				// 				<div class="text-center">
				// 					<button type="button" class="btn btn-xs btn-outline-primary btn-block" onclick="vfile('` + row[15] + `')"><i class="fa fa-eye" aria-hidden="true"></i> View</button>
				// 					<a class="btn btn-xs btn-outline-dark btn-block" href="<?php echo base_url("assets/uploads/foto/cover/"); ?>` + row[15] + `" download><i class="fa fa-download" aria-hidden="true"></i> Download</a>
				// 				</div>
				// 			`;
				// 		} else {
				// 			return `
	...
	function delbuku(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: "<?php echo site_url('buku_ajar/delete'); ?>",
					type: "POST",
					data: "id=" + id,
					cache: false,
					dataType: 'json',
	...
	function update_status(id, status) {
		$.ajax({
			url: "<?php echo site_url('buku_ajar/update_status'); ?>",
			type: "POST",
			data: {
				id: id,
				status: status
	...
	function edit_buku(id) {
		...
		$.ajax({
			url: "<?php echo site_url('buku_ajar/edit_buku') ?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data) {
	...
	function save() {
		...
		if (save_method == 'add') {
			url = "<?php echo site_url('buku_ajar/insert') ?>";
		} else {
			url = "<?php echo site_url('buku_ajar/update') ?>";
		}
