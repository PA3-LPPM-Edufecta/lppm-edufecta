			...
			<div class="col-12">
				<div class="card">
					<div class="card-header bg-light elevation-1"">
						<h3 class=" card-title mt-1"><i class="fa fa-list text-blue"></i> Data HKI</h3>
						<!-- Card-Tools -->
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $("#tbl_hki").DataTable({
			...
			"ajax": {
				"url": "<?php echo site_url('HKI_penelitian/ajax_list') ?>",
				"type": "POST"
			},
			...
			"columnDefs": [{
				...
				},
				{
					targets: [7],
					render: function(data, type, row, meta, date) {
						return moment(new Date(row[11]).toString()).format('DD-MM-YYYY');
					}
				},
				{
					targets: [8],
					render: function(data, type, row, meta, date) {
						// if (row[12] != null) {
						// 	return "<img class=\"img img-responsive img-thumbnail\"  src='<?php echo base_url("assets/uploads/foto/cover/"); ?>" + row[12] + "' width=\"100px\" height=\"100px\">";
						// } else {
						// 	return "<img class=\"myImgx\"  src='<?php echo base_url("assets/uploads/foto/default-150x150.png"); ?>' width=\"100px\" height=\"100px\">";
						// } <a href="<?php echo base_url("assets/uploads/foto/cover/"); ?>` + row[12] + `" aria-haspopup="true" aria-expanded="false" class="text-center">Download</a>
						// <a class="btn btn-xs btn-outline-primary" href="<?php echo base_url("assets/uploads/foto/cover/"); ?>` + row[12] + `" data-toggle="lightbox" data-title="View"><i class="fa fa-eye" style="color: #2c2ce6;" aria-hidden="true"></i></a>
						/*
	...
	function vhki(id) {
		...
		$.ajax({
			url: '<?php echo base_url('HKI_penelitian/viewhki'); ?>',
			type: 'post',
	...
	function delhki(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: "<?php echo site_url('HKI_penelitian/delete'); ?>",
					type: "POST",
					data: "id=" + id,
					cache: false,
					dataType: 'json',
	...
	function update_status(id, status) {
		$.ajax({
			url: "<?php echo site_url('HKI_penelitian/update_status'); ?>",
			type: "POST",
			data: {
				id: id,
				status: status
	...
	function edit_hki(id) {
		...
		$.ajax({
			url: "<?php echo site_url('HKI_penelitian/edit_hki') ?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data) {
				...
				$('[name="nomor_hki"]').val(data.nomor_hki);
				...
				$('.modal-title').text('Edit HKI'); // Set title to Bootstrap modal title
			},
	...
	function save() {
		...
		if (save_method == 'add') {
			url = "<?php echo site_url('HKI_penelitian/insert') ?>";
		} else {
			url = "<?php echo site_url('HKI_penelitian/update') ?>";
		}
				...
				<form action="#" id="form" class="form-horizontal">
					...
					<div class="card-body">
						...
						<div class="form-group row ">
							...
							<div class="col-sm-9 kosong">
								<input type="text" class="form-control" name="nomor_pendaftaran" id="nomor_pendaftaran" placeholder="Nomor Pendaftaran">
								<span class="help-block"></span>
							</div>
						</div>
						<div class="form-group row ">
							<label for="nomor_hki" class="col-sm-3 col-form-label">Nomor HKI</label>
							<div class="col-sm-9 kosong">
								<input type="text" class="form-control" name="nomor_hki" id="nomor_hki" placeholder="Nomor HKI">
								<span class="help-block"></span>
							</div>
						</div>
						<div class="form-group row ">
