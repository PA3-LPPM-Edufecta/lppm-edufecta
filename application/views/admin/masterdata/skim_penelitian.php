			...
			<div class="col-12">
				<div class="card">
					<div class="card-header bg-light">
						<h3 class="card-title mt-1"><i class="<?php echo $this->db->get('tbl_submenu')->row(6)->icon; ?> text-blue"></i> Data Skim Penelitian</h3>
						<!-- Card-Tools -->
						<div class="card-tools">
							<button type="button" class="btn btn-tool text-blue" data-card-widget="card-refresh" data-source="/lppm/skim_penelitian" data-source-selector="#card-refresh-content" data-load-on-init="false">
					...
					<div class="card-body">
						<div class="text-left mb-4">
							<button type="button" class="btn btn-sm btn-outline-primary" onclick="add_skim_penelitian()" title="Add Data"><i class="fas fa-plus"></i> Add</button>
						</div>
						<table id="tbl_skim_penelitian" class="table table-bordered table-striped table-hover">
							<thead>
								<tr class="bg-blue">
									<th width="50">No</th>
									<th>Nama</th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		//datatables
		table = $("#tbl_skim_penelitian").DataTable({
			"responsive": true,
			...
			"autoWidth": false,
			"language": {
				"sEmptyTable": "Data Skim Penelitian Belum Ada"
			},
			...
			"ajax": {
				"url": "<?php echo site_url('skim_penelitian/ajax_list') ?>",
				"type": "POST"
			},
					...
					"render": function(data, type, row) {
						...
						if(row[7]==0){
							return `
								...
								<li><a href="javascript:void(0)" class="dropdown-item" title="Edit" data-role="edit" onclick="edit_skim_penelitian(`+row[8]+`)">Edit</a></li>
								<li><a href="javascript:void(0)" class="dropdown-item" title="Delete" nama=" + row[0] + "  onclick="delskimpenelitian(`+row[8]+`)">Hapus</a></li>
	...
	function delskimpenelitian(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: "<?php echo site_url('skim_penelitian/delete'); ?>",
					type: "POST",
					data: "id=" + id,
					cache: false,
					dataType: 'json',
	...
	function add_skim_penelitian() {
		...
		$('.modal-title').text('Add Skim Penelitian'); // Set Title to Bootstrap modal title
	}

	function edit_skim_penelitian(id) {
		save_method = 'update';
		...
		$.ajax({
			url: "<?php echo site_url('skim_penelitian/edit_skim_penelitian') ?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data) {
				...
				$('.modal-title').text('Edit Skim Penelitian'); // Set title to Bootstrap modal title
	...
	function update_status(id, status){
		$.ajax({
			url: "<?php echo site_url('skim_penelitian/update_status'); ?>",
			type: "POST",
	...
	function save() {
		...
		if (save_method == 'add') {
			url = "<?php echo site_url('skim_penelitian/insert') ?>";
			data = $('#form').serialize();
		} else {
			url = "<?php echo site_url('skim_penelitian/update') ?>";
		...
		<div class="modal-content ">
			...
			<div class="modal-body form">
				<form action="#" id="form" class="form-horizontal">
                    ...
                    <input type="hidden" value="1" name="id_tipe_pengajuan" />
					<div class="card-body">
