				...
				<div class="card">
					<div class="card-header bg-light elevation-1">
						...
						<div class="card-tools">
							<button type="button" class="btn btn-tool text-blue" data-card-widget="card-refresh" data-source="/lppm/luaran_lain_pengabdian" data-source-selector="#card-refresh-content" data-load-on-init="false">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $("#tbl_luaran").DataTable({
			...
			"ajax": {
				"url": "<?php echo site_url('luaran_lain_pengabdian/ajax_list') ?>",
				"type": "POST"
			},
	...
	function vluaran(id) {
        ...
        $.ajax({
          url: '<?php echo base_url('luaran_lain_pengabdian/viewluaran'); ?>',
          type: 'post',
	...
	function delluaran(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: "<?php echo site_url('luaran_lain_pengabdian/delete'); ?>",
					type: "POST",
					data: "id=" + id,
					cache: false,
					dataType: 'json',
	...
	function update_status(id, status) {
		$.ajax({
			url: "<?php echo site_url('luaran_lain_pengabdian/update_status'); ?>",
			type: "POST",
			data: {
				id: id,
				status: status
	...
	function edit_luaran(id) {
		...
		$.ajax({
			url: "<?php echo site_url('luaran_lain_pengabdian/edit_luaran') ?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data) {
	...
	function save() {
		...
		if (save_method == 'add') {
			url = "<?php echo site_url('luaran_lain_pengabdian/insert') ?>";
		} else {
			url = "<?php echo site_url('luaran_lain_pengabdian/update') ?>";
		}
